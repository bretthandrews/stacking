...
for filelist in filelists:
    ...
    grid = make_grid()
    stacks = np.zeros((samples, len(grid)))
    ...
    assert np.abs(stack - stack_comp).max() < 1e-6, 'stack and comparison stack differ'
...
fig, ax = plt.subplots()
ax.plot(grid, spec_mean, c=c0)
ax.fill_between(grid, spec_mean - spec_std, spec_mean + spec_std, facecolor=c0, alpha=0.5)
